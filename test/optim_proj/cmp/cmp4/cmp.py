...
def main():
    """test for Sparse AE"""
    T = []
    T1 = SparseAE(64, 49, optimize_method='bfgs', max_iter=500,
                  debug=0, verbose=True, tol=1e-8, mini_batch=32,
                  logger='bfgs.csv')
    T2 = SparseAE(64, 49, optimize_method='cg', max_iter=500,
                  debug=0, verbose=True, tol=1e-8, mini_batch=32,
                  logger='cg.csv')
    T3 = SparseAE(64, 49, optimize_method='sgd', max_iter=180,
    ...
    T.append(T2)
    T.append(T3)
    X = vs.load_sample('IMAGES25.mat', patch_size=8, n_patches=60000)
