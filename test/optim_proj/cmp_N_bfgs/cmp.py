...
def main():
    ...
    T1 = SparseAE(64, 49, optimize_method='bfgs', max_iter=200,
                  ...
                  logger='bfgs_good.csv')
    T2 = SparseAE(64, 49, optimize_method='bfgs', max_iter=200,
                  debug=0, verbose=True, tol=1e-8, mini_batch=64,
                  momentum=0, momen_beta=.95, alpha=.01, adastep=1,
                  logger='bfgs_ill.csv')
    T3 = SparseAE(64, 49, optimize_method='bfgs', max_iter=200,
                  debug=0, verbose=True, tol=1e-8, mini_batch=64,
                  momentum=0, momen_beta=.95, alpha=.01, adastep=1,
                  logger='bfgs_worse.csv')
    T4 = SparseAE(64, 49, optimize_method='bfgs', max_iter=200,
                  debug=0, verbose=True, tol=1e-8, mini_batch=64,
                  momentum=0, momen_beta=.95, alpha=.01, adastep=1,
                  logger='bfgs_worst.csv')
    T.append(T1)
    T.append(T2)
    T.append(T3)
    T.append(T4)
